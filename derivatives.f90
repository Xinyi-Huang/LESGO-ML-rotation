...
do jz=lbz,nz
   ...
   ! Xinyi - comment : see the comment below for filt_da. We should be careful
   ! along x direction.
   dfdx(ld-1:ld,:,jz)=0._rprec
   dfdx(:,ny/2+1,jz)=0._rprec
...
do jz=lbz,nz
  ...
  ! Xinyi - comment : This process will change the value of f, or u,v,w in
  ! main.f90, which might be not expected. At the same time, along x direction,
  ! we should not deal with ld-1:ld after fft to avoid aliasing. Such problem
  ! also occurs in ddxy
