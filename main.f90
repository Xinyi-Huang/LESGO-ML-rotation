...
use param
use sim_param
!use wmrotate_param, only : wmro_nstep  ! Xinyi - possible control for step num
use grid_m
...
! Xinyi - debug : what is utau
real(rprec) :: utaup, utaus, nu_debug, yplus
...
allocate ( up_swm(nx,ny) )
up_swm=0.0_rprec !xiang dirty swm for dudt
allocate ( vp_swm(nx,ny) )
vp_swm=0.0_rprec !xiang dirty swm for dudt

! Xinyi - debug : nu for debug
nu_debug = nu_molec/(z_i*u_star)
...
time_loop: do jt_step = nstart, nsteps   
   ...
   tt=tt+dt
    ...
    if (dns_bc) then
        ...
        if (coord == 0) then
            !call wallstress ()
            !call wallstress_wmro ()    ! Xinyi - WM - rotate : wall stress calculation
            call wallstress_ML ()       ! Xinyi - WM - MLrot  : wall stress calculation
            ! Xinyi - debug : output utaup and yplus
            utaup = sqrt(sum( - txz(1:nx,1:ny,1)/(nx*ny) ))
            yplus = 0.5_rprec*dz*utaup/nu_debug
            print *, 'utaup, yplus : ', utaup, yplus
        end if
        if (coord == nproc-1) then     ! Xinyi's upper boundary layer
            !call wallstress_upper ()
            call wallstress_upper_wmro () ! Xinyi - WM - rotate : wall stress calculation
            ! Xinyi - debug : output utaup and yplus
            utaus = sqrt(sum( txz(1:nx,1:ny,nz)/(nx*ny) ))
            yplus = 0.5_rprec*dz*utaus/nu_debug
            print *, 'utaus, yplus : ', utaus, yplus
        end if
    end if    
    ...
    if (coriolis_forcing) then
        ...
        RHSx(:, :, 1:nz-1) = RHSx(:, :, 1:nz-1) +                 &
                        coriol(3) * v(:, :, 1:nz-1) -             &
                        coriol(2) * (w(:, :, 1:nz-1) + w(:, :, 2:nz)) /2
        RHSy(:, :, 1:nz-1) = RHSy(:, :, 1:nz-1) +                 &
                        coriol(1) * (w(:, :, 1:nz-1) + w(:, :, 2:nz)) /2 - &
                        coriol(3) * u(:, :, 1:nz-1)
        ! Xinyi - WM - rotate : Here we assume no-slip boundary condition, so
        ! that we do not have to change the u(:,:,0) and v(:,:,0) from 0 to
        ! -u(:,:,1), -v(:,:,1)
        if (coord > 0) then
        RHSz(:, :, 1:nz-1) = RHSz(:, :, 1:nz-1) +                 &
                        coriol(2) * (u(:, :, 1:nz-1) + u(:, :, 0:nz-2)) /2 - &
                        coriol(1) * (v(:, :, 1:nz-1) + v(:, :, 0:nz-2)) /2
        else
        RHSz(:, :, 2:nz-1) = RHSz(:, :, 2:nz-1) +                 &
                        coriol(2) * (u(:, :, 2:nz-1) + u(:, :, 1:nz-2)) /2 - &
                        coriol(1) * (v(:, :, 2:nz-1) + v(:, :, 1:nz-2)) /2
        end if
    end if
