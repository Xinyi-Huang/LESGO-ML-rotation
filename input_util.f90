...
do 
  ...
  if( block_exit_pos == 0 ) then
     ...
     case ('CORIOL_X')
        read (buff(equal_pos+1:), *) coriol(1)
     case ('CORIOL_Y')
        read (buff(equal_pos+1:), *) coriol(2)
     case ('CORIOL_Z')
        read (buff(equal_pos+1:), *) coriol(3)
     case ('UG')
  ...
  elseif( block_exit_pos == 1 ) then
     ...
     ! Xinyi - suggestion : Check coriolis force again in case using it
     ! inadvertently
     if(.not. coriolis_forcing) coriol = 0.0_rprec
...
use wmrotate_param, only : wmro_z_n, wmro_z_ni, wmro_z_r ! Xinyi - WM - rotate 
...
do 
  ...
  if( block_exit_pos == 0 ) then
     ...
     case ('UBC_MOM')                            ! Xinyi's flag for upper boundary layer
        Read (buff(equal_pos+1:), *) ubc_mom
     case ('IWM_ACTIVE')                     ! Xiang flag for integral wall model
        Read (buff(equal_pos+1:), *) iwm_on  
     case ('WMRO_Z_N')                          ! Xinyi - WM - rotate
        Read (buff(equal_pos+1:), *) wmro_z_n
        wmro_z_ni = 2*wmro_z_n + 1     
     case ('WMRO_Z_R')                          ! Xinyi - WM - rotate
        Read (buff(equal_pos+1:), *) wmro_z_r
     case ('ZO')
        read (buff(equal_pos+1:), *) zo
     case ('INFLOW')
        read (buff(equal_pos+1:), *) inflow
     case ('INITIAL_UNIFORM')  ! Xinyi - Initial condition for using uniform inflow
        read (buff(equal_pos+1:), *) initial_uniform
     ...
     case ('INITIAL_VELOCITY_CONST')      ! Xinyi : initial velocity scaling constant
        read (buff(equal_pos+1:), *) initial_velocity_const
  ...
  elseif( block_exit_pos == 1 ) then
     ...
     if( use_mean_p_force .AND. eval_mean_p_force ) then
        ...
        ! Xinyi - pressure change
        ! Evaluate the mean pressure force
        mean_p_force = mean_p_force / L_z
        if( ubc_mom ) mean_p_force = mean_p_force * 2.0_rprec
