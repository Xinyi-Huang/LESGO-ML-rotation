...
real (rp) :: phix
real (rp) :: dmin
real (rp) :: z, z_totop     ! Xinyi : to top distance
    ...
    do i = 1, nx
      ...
      if (phix > 0._rp) then
        ...
        if (lbc_mom == 0) then
           ! Stress free
           dmin = phix
        elseif (ubc_mom == 0) then    ! Xinyi's upper boundary
           ! Stress free
           z = (k - 1) * dz
           dmin = min (z, phix)
        else                          ! Xinyi's upper boundary
           z = (k - 1) * dz
           z_totop = ((nproc - coord) * (nz - 1) - (k - 1)) * dz
           dmin = min (z, z_totop, phix)
        end if
    ...
    do i = 1, nx
      ...
      if ((phi_a < phi_x) .and. (phi_x < phi_b)) then
        ...
        if (mag (v1t) < eps) then
          ...
          !  call string_splice( fname, path // 'output/LS_stress')
          !  call string_concat( fname, '.c', coord )
          !  open(unit=134,file=fname,form='formatted',status='unknown',position='append')
          !  write(134) jt_total, x, i, j, k,' txy ',txy(i,j,k)
          !  close(134)
          !end if
    ...
    do i = 1, nx
      ...
      if ((phi_a < phi_x) .and. (phi_x < phi_b)) then
        ...
        if (mag (v1t) < eps) then
          ...
          !  call string_concat( fname, '.c', coord )
          !  open(unit=134,file=fname,form='formatted',status='unknown',position='append')
          !  write(134) jt_total, x, i, j, k, 'txz', txz(i,j,k)
          !  close(134)
          !endif
...
! Xinyi - warning : both nphibot and nphitop +1, to allow interpolation a little
! bit out of range
if ( coord == 0 ) then
  if (ku < 1) call error (sub_name, 'ku out of range, ku =', ku)
else
  if (ku < -(nphibot+1)) call error (sub_name,                     &
                                 'out of range, (ku, kumin) =',  &
                                 (/ ku, -(nphibot+1) /))
end if
  ...
  if (coord == nproc-1) then
    ...
    if (ku > nz + nphitop) call error (sub_name,                      &
                                         'out of range (ku, kumax) =',  &
                                         (/ ku, nz + nphitop /))
  endif
#else
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
use param, only : jt_total, dt, L_y, L_z, nz
...
real (rp) :: f_Cxz, f_Cxz_global !xiang
integer :: i, j, k !xiang
!---------------------------------------------------------------------
...
do j=1,ny
do k=1,nz-1
  f_Cxz=f_Cxz - fx(i,j,k)*dx*dy*dz*(dz*(coord*(nz-1)+k-0.5_rprec))
end do
end do
end do 
...
Uinf = sum (u(1, :, 1:nz-1)) / (ny * (nz - 1))  !--measure at inflow plane
  ...
  call mpi_reduce (Uinf, Uinf_global, 1, MPI_RPREC, MPI_SUM,  &
                   rank_of_coord(0), comm, ierr)
  call mpi_reduce (f_Cxz,f_Cxz_global,1, MPI_RPREC, MPI_SUM,  &
                   rank_of_coord(0), comm, ierr) !xiang
...
if( coord == 0 ) then
  ...
  if( .not. file_init ) then
    ...
    if (.not. exst) then
        ...
        write(fid,*) '"t", "CxA", "fx", "CyA", "fy", "CzA", "fz", "Uinf", "fxz"'
        close(fid)
    end if
    ...
    write(fid,*) total_time, CxA, f_Cx_global, CyA, f_Cy_global, CzA, f_Cz_global, Uinf_global, f_Cxz_global !xiang: added f_Cxz_global
    close(fid)
...
integer :: jx, jy, jz
integer :: jz_min, jz_max         ! Xinyi : jz max
integer :: node

logical, save :: initialized = .false.

real (rp) :: dmin, z, z_totop     ! Xinyi : to top
real (rp) :: phi_tmp
...
if (.not. initialized) then
  ...
  if (coord == nproc - 1) then   ! Xinyi : upper boundary
    ...
    do jy = 1, ny
      do jx = 1, nx
        ...
        if (ubc_mom == 0) then   ! Xinyi : upper boundary
           ! Stress free
           z = (jz - 0.5_rp) * dz
           dmin = min (phi(jx, jy, jz), z)
        else
           ...
           z = (jz - 0.5_rp) * dz
           z_totop = (nz - 0.5_rp - jz) * dz
           dmin = min (phi(jx, jy, jz), z, z_totop)
  ...
  do jz = jz_min, jz_max
    do jy = 1, ny
      do jx = 1, nx
        ...
        elseif (ubc_mom == 0) then   ! Xinyi : upper boundary
           ! Stress free
#ifdef PPMPI
...
#ifdef PPMPI
            z = (coord * (nz - 1) + jz - 1) * dz
            z_totop = ((nproc - coord) * (nz - 1) - (jz - 1)) * dz
#else
            z = (jz - 1) * dz
            z_totop = (nz - 1 - (jz - 1)) * dz
#endif
          
          phi_tmp = 0.5_rp * (phi(jx, jy, jz) + phi(jx, jy, jz - 1))
                              !--MPI: requires phi(k=0)
          
          dmin = min (phi_tmp, z, z_totop)  !--min distance to surface
